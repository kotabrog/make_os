...
impl GenericAddress {
    pub fn address_in_memory_space(&self) -> Result<usize> {
        if self.address_space_id == 0 {
            ...
            Err("ACPI Generic Address is not in memory space")
        }
    }
}
...
impl AcpiHpetDescriptor {
    pub fn base_address(&self) -> Result<&'static mut HpetRegister> {
        unsafe {
            self.address
                .address_in_memory_space()
                .map(|addr| &mut *(addr as *mut HpetRegister))
        }
    }
}
