...
impl BusDeviceFunction {
    ...
    pub fn fmt_common(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(
            f,
            "/pui/bus/{:#04X}/device/{:#04X}/function/{:#03X}",
            self.bus(),
            self.device(),
            self.function()
        )
...
impl<T> ConfigRegister<T> {
    ...
    fn write(ecm_base: *mut T, byte_offset: usize, data: T) -> Result<()> {
        if !(0..256).contains(&byte_offset) || byte_offset % size_of::<T>() != 0 {
            Err("PCI ConfigRegister write out of range")
        } else {
            unsafe {
...
impl Pci {
    ...
    pub fn read_register_u16(&self, bdf: BusDeviceFunction, byte_offset: usize) -> Result<u16> {
        ConfigRegister::read(self.ecm_base(bdf), byte_offset)
    }

    pub fn read_vendor_device_id(&self, bdf: BusDeviceFunction) -> Option<VendorDeviceId> {
        let vendor = self.read_register_u16(bdf, 0).ok()?;
        let device = self.read_register_u16(bdf, 2).ok()?;
    ...
    pub fn probe_devices(&self) {
        for bdf in BusDeviceFunction::iter() {
            if let Some(vd) = self.read_vendor_device_id(bdf) {
                info!("{vd}");
    ...
    pub fn write_register_u32(
        ...
        ConfigRegister::write(self.ecm_base(bdf), byte_offset, data)
    }
    ...
    pub fn set_command_and_status_flags(
        ...
        let cmd_and_status = self.read_register_u32(bdf, 0x04 /* Comannd and status */ )?;
        self.write_register_u32(
            bdf,
            0x04 /* Comannd and status */ ,
